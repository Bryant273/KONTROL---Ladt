...
export default function Logistics() {
  ...
  const [activePage, setActivePage] = useState<string>('log-tdb');
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex justify-between items-end">
              <div>
                ...
                <p className="text-xs text-slate-400">Gestion de la Logistique & Stocks — Mai 2026</p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <StatCard icon={Box} label="Articles" value="1,250" />
              <StatCard icon={Truck} label="Livraisons" value="12" badge="+2" badgeType="pos" />
              <StatCard icon={History} label="Mouvements" value="245" />
              <StatCard icon={AlertCircle} label="Alertes Stock" value="4" badge="Urgent" badgeType="neg" />
            </div>
            <div className="bg-white rounded-2xl border border-black/5 shadow-sm p-6 overflow-x-auto">
              <h3 className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-6">État des stocks</h3>
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100">
                    <th className="text-left py-3 px-4 text-[10px] font-bold uppercase tracking-wider text-slate-400">Ref Produit</th>
                    <th className="text-left py-3 px-4 text-[10px] font-bold uppercase tracking-wider text-slate-400">Désignation</th>
                    <th className="text-right py-3 px-4 text-[10px] font-bold uppercase tracking-wider text-slate-400">Qté</th>
                    <th className="text-right py-3 px-4 text-[10px] font-bold uppercase tracking-wider text-slate-400">Alerte</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50 text-[13px]">
                  <StockRow sku="ITM-001" name="Rame Papier A4 80g" qte="450" unit="Rame" alert={false} />
                  <StockRow sku="ITM-002" name="Cartouche Encre HP 305" qte="12" unit="Unité" alert={true} />
                  <StockRow sku="ITM-003" name="Gel Hydroalcoolique 1L" qte="85" unit="Flacon" alert={false} />
                </tbody>
              </table>
            </div>
          </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
          <SidebarLink 
            ...
            active={activePage === 'log-tdb'} 
            onClick={() => setActivePage('log-tdb')} 
          />

          <SidebarSection 
            label="Stocks" 
            isOpen={openSections.has('stocks')} 
            onToggle={() => toggleSection('stocks')}
          >
            <SidebarLink icon={Package} label="Inventaire" active={activePage === 'inventory'} onClick={() => setActivePage('inventory')} />
            <SidebarLink icon={Box} label="État du stock" active={activePage === 'stock_level'} onClick={() => setActivePage('stock_level')} />
            <SidebarLink icon={Layout} label="Catalogue" active={activePage === 'catalog'} onClick={() => setActivePage('catalog')} />
          </SidebarSection>

          <SidebarSection 
            label="Mouvements" 
            isOpen={openSections.has('mouvements')} 
            onToggle={() => toggleSection('mouvements')}
          >
            <SidebarLink icon={Pencil} label="Bon d'entrée" active={activePage === 'in_move'} onClick={() => setActivePage('in_move')} />
            <SidebarLink icon={Scan} label="Bon de sortie" active={activePage === 'out_move'} onClick={() => setActivePage('out_move')} />
            <SidebarLink icon={Truck} label="Expéditions" active={activePage === 'shipping'} onClick={() => setActivePage('shipping')} />
          </SidebarSection>
        </motion.nav>
...
function StockRow({ sku, name, qte, unit, alert }: any) {
  return (
    <tr className="hover:bg-slate-50 cursor-pointer transition-colors group">
      <td className="py-4 px-4 font-mono font-bold text-slate-900 group-hover:text-brand transition-colors">{sku}</td>
      <td className="py-4 px-4 font-medium text-slate-700">{name}</td>
      <td className="py-4 px-4 text-right">
        <span className="font-bold text-slate-900">{qte}</span>
        <span className="text-[10px] text-slate-400 ml-1.5 uppercase font-medium">{unit}</span>
      </td>
      <td className="py-4 px-4 text-right">
        {alert ? (
          <span className="bg-rose-50 text-rose-600 px-2.5 py-1 rounded-lg text-[10px] font-bold">Critique</span>
        ) : (
